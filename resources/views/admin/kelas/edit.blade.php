...
<div class="p-6 w-full h-full bg-white/60 rounded-xl shadow">
    ...
    <form action="{{ route('admin.kelas.update', $kelas->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="nama_kelas" value="{{ old('nama_kelas', $kelas->nama_kelas) }}" class="w-full border p-2 rounded" required>
        </div>
        <div>
            <label>Deskripsi</label>
            <textarea name="deskripsi" class="w-full border p-2 rounded">{{ old('deskripsi', $kelas->deskripsi) }}</textarea>
        </div>
        <div>
            <label>Gambar</label>
            @if($kelas->gambar)
                <img src="{{ asset('storage/'.$kelas->gambar) }}" alt="Gambar Kelas" class="w-32 mb-2">
            @endif
            <input type="file" name="gambar" class="w-full border p-2 rounded">
        </div>
        <div>
            <label>Harga</label>
            <input type="number" name="harga" value="{{ old('harga', $kelas->harga) }}" class="w-full border p-2 rounded" required>
        </div>
        <div>
            <label>Status</label>
