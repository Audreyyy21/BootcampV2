...
<form action="{{ route('admin.jadwal.store') }}" method="POST" class="space-y-5">
    ...
    <div>
        ...
        <input type="date" name="tanggal_mulai" id="tanggal_mulai" value="{{ old('tanggal_mulai') }}" class="w-full border-gray-300 rounded-lg focus:ring-yellow-400">
    </div>

    {{-- Tanggal Selesai --}}
    <div>
        <label for="tanggal_selesai" class="block text-gray-700 font-medium mb-2">Tanggal Selesai</label>
        <input type="date" name="tanggal_selesai" id="tanggal_selesai" value="{{ old('tanggal_selesai') }}" class="w-full border-gray-300 rounded-lg focus:ring-yellow-400">
    </div>
    ...
    <input type="url" name="gmeet_link"
        value="{{ old('gmeet_link', $jadwal->gmeet_link ?? '') }}"
        placeholder="https://meet.google.com/xxx-yyyy-zzz"
        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring focus:border-yellow-400">
</div>
    ...
    <div class="flex justify-end gap-3 mt-6">
        ...
        <button type="submit" class="bg-yellow-500 text-white px-5 py-2 rounded-lg hover:bg-yellow-600 transition">Simpan</button>
    </div>
</form>
</div>
